            ...
            for assignment in factor.getAllPossibleAssignmentDicts():
                ...
                if assignment[GHOST_HOUSE_VAR] == current:
                    if assignment[obsVar]==RED_OBS_VAL:
                        ...
                        prob = 0
                    factor.setProbability(assignment, prob)
                if assignment[FOOD_HOUSE_VAR] == current:
                    if assignment[obsVar]==RED_OBS_VAL:
                        prob = PROB_FOOD_RED
                    elif assignment[obsVar]==BLUE_OBS_VAL:
...
def getMostLikelyFoodHousePosition(evidence, bayesNet, eliminationOrder):
    ...
    """
    "*** YOUR CODE HERE ***"
    eliminatedFactor = inference.inferenceByVariableElimination(bayesNet, 'FoodHouse', evidence, eliminationOrder)
    mostLikely = 0
    bestLocation = None
...
class VPIAgent(BayesAgent):
    ...
    def computeEnterValues(self, evidence, eliminationOrder):
        ...
        *Do not* take into account the "time elapsed" cost of traveling to each
        ...
        evidence = all configuration of the world. example: given the world is position[a,b]=red 
        position[c,d]=blue... what's the expected value of entering left house immediately.

        p(Enterleft| evidence)
        """
        ...
        "*** YOUR CODE HERE ***"
        # p(foodhouse, ghosthouse |evidence)
        worldInference = inference.inferenceByVariableElimination(self.bayesNet, HOUSE_VARS, evidence, eliminationOrder)
        p_food_left_ghost_right =0
        p_food_right_ghost_left = 0

        for assignment in worldInference.getAllPossibleAssignmentDicts():
            if assignment['foodHouse']==TOP_LEFT_VAL and assignment['ghostHouse']==TOP_RIGHT_VAL:
                p_food_left_ghost_right = worldInference.getProbability(assignment)
            elif assignment['foodHouse']==TOP_RIGHT_VAL and assignment['ghostHouse']==TOP_LEFT_VAL:
                p_food_right_ghost_left = worldInference.getProbability(assignment)

        leftExpectedValue = p_food_left_ghost_right* WON_GAME_REWARD + (1-p_food_left_ghost_right)*GHOST_COLLISION_REWARD
        rightExpectedValue = p_food_right_ghost_left * WON_GAME_REWARD + (1- p_food_right_ghost_left)*GHOST_COLLISION_REWARD
    ...
    def computeExploreValue(self, evidence, enterEliminationOrder):
        ...
        for prob, explorationEvidence in self.getExplorationProbsAndOutcomes(evidence):
            enterLeft, enterRight = self.computeEnterValues(explorationEvidence, enterEliminationOrder)
            highestExpected = max(enterLeft, enterRight)
            expectedValue += highestExpected * prob
